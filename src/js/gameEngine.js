...
function gameLoop(state, game, timestamp) {
  ...
  if (timestamp > state.green.nextSpawnTimestamp) {
    ...
    state.green.nextSpawnTimestamp =
      timestamp + Math.random() * state.green.maxSpawnInterval;
  }
  ...
  if (timestamp > state.red.nextSpawnTimestamp) {
    ...
    state.red.nextSpawnTimestamp =
      timestamp + Math.random() * state.red.maxSpawnInterval;
  }
  ...
  document.querySelectorAll(".laser").forEach((laser) => {
    ...
    robotElements.forEach((robot) => {
      if (detectCollision(robot, laser)) {
        const audio = new Audio("/src/assets/blast.mp3");
        audio.play();
        state.score += state.killScore;
        robot.style.backgroundImage = 'url("/src/assets/blast.png")';
        setTimeout(() => {
          robot.remove();
        }, "250");
        laser.remove();
    ...
    startX > game.gameScreen.offsetWidth
      ? laser.remove()
      : (laser.style.left = startX + state.laser.speed + "px");
  });
